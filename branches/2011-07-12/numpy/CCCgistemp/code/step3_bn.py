...
def iter_subbox_grid(station_records, max_months, first_year, radius):
    ...
    *max_months* is the maximum number of months in any station
    ...
    misses = np.repeat(np.nan, max_months)
    ...
    for region in regions:
        ...
        for subbox in subboxes:
            ...
            a = record.series_array # just a temporary
            subbox_series[offset:offset + len(a)] = a

            max_weight = wt

            weight = wt * ~np.isnan(subbox_series)
            ...
            l = [(~np.isnan(subbox_series[i::12])).any() for i in range(12)]
            ...
            for record,wt in contributors[1:]:
                ...
                new[aa - 1:bb] = record.series_array
                station_months = series.combine_bn(subbox_series,
                                                   weight, new, wt,
            ...
            #NOTE: ?
            if 1:
                subbox_series[subbox_series==np.nan] = 9999.0
                subbox_series = subbox_series.tolist()
